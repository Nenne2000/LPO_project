...
public class BufferedTokenizer implements Tokenizer {
	...
	static { // initialization of the symbol and keyword tables: symbols and keywords are singleton lexical categories 
		...
		symbols.put("&&", AND);
		symbols.put("==", EQ);
		symbols.put("!=", NEQ); //SIMBOLO NEQ (NOT EQUALS)
		...
		symbols.put("[", OPEN_RANGE); //SIMBOLO PER '['
		symbols.put("]", CLOSE_RANGE); //SIMBOLO PER ']'
		symbols.put(":", COLON); //SIMBOLO PER ':'
		...
		keywords.put("for", FOR); //KEYWORD FOR
		keywords.put("in", IN); //KEYWORD IN
		keywords.put("bounds", BOUNDS); //KEYWORD BOUNDS
	}
